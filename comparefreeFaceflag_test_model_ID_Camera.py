...
def combinations_self(ID_list,Camera_list):
    mix_data=[]
    for ID in ID_list:
        for camera in Camera_list:
            mix_data.append((ID,camera))
    return mix_data
...
if __name__ == '__main__':
    ...
    P_valid_force = 3000        # sample number need to change
    N_valid_force = 14560       #100w
    best_precison = 0
    best_accuracy = 0
    ...
    baseData_ID = baseImageRep_tf(test_file_ID)  # 提取ID基础集的特征点和标签
    baseData_camera = baseImageRep_tf(test_file_camera)  # 提取camera基础集的特征点和标签
