...
if __name__ == '__main__':
    ...
    P_valid_force = 100000        # sample number need to change
    ...
    savedir = os.path.join('./saveResults/',now+argv[2])
    ...
    # plt.show()
    f2.write('threshold, acc, recall, fpr, precision, tnr, fnr\n')
    list_data = ['threshold', 'acc', 'recall', 'fpr', 'precision', 'tnr', 'fnr']
    for index in range(len(list_data)):
        sheet_1.write(row_count, index, list_data[index])
    ...
    for threshold in thresholds:
        ...
        ss = 'recall:'+str(recall)+'\t'+'precison:'+str(precision)+'\t'+' accuracy:'+str(acc)+'\n'
        sss = 'TNR:'+str(tnr)+'\t'+'FPR:'+str(fpr)+'\t'+'FNR:'+str(fnr)+'\t\n\n'
        print ss,sss
        f1.write(ss)
        f1.write(sss)
        f2.write('%f,%.9f,%.9f,%.9f,%.9f,%.9f,%.9f\n'%(threshold, acc, recall, fpr, precision, tnr, fnr))
        list_data=[threshold, acc, recall, fpr, precision, tnr, fnr]
